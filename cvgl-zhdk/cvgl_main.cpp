...
int main( void )
{
    ...
    if( bm_cam.hasCamera() )
    {
        ...
        app.context.setupWindow( bm_cam.getWidth(), bm_cam.getHeight(), title );
        bm_cam.start();
    }
    ...
    if( bm_cam2.hasCamera() )
    {
       ...
       if( !bm_cam.hasCamera() )
       {
           app.context.setupWindow( bm_cam2.getWidth(), bm_cam2.getHeight(), title );
           app.useCameraID(2);
       }
    }
    ...
    if( !bm_cam.hasCamera() && !bm_cam2.hasCamera() )
    {
        if(cvcam.hasCamera())
        {
            app.context.setupWindow( cvcam.getWidth(), cvcam.getHeight(), title );
            app.useCameraID(3);
        }
        else
        {
