...
MapOSC cue_moon_hands_fadeout(cueArgs args)
{
    ...
    double fadetime = 20;
    double fadeOutTime = fadetime * 0.5;
    if( elapsed_section <= fadetime )
    {
        if( elapsed_section < fadeOutTime )
        {
            out.addMessage("/korg/pregain/dB",  scale(elapsed_section, 0., fadeOutTime, -0., -36));
        }
        else
        {
            out.addMessage("/korg/pregain/dB",  scale(elapsed_section, fadeOutTime, fadetime, -36., -6));
