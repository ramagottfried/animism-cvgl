...
MapOSC cue_leaf_in_slug(cueArgs args)
{
    ...
    if( !isNewCue && data.ncontours > 0 )
    {
     ...
     //   double area_avg = data.contour_area.mean();
        ...
        out.addMessage("/dpo/sarah/amp", pow(sin_area_phase, 2), 20 ); // area_sum * area_sum + scale(dist_sum, dmin, dmax, 0.00, 1 - area_sum)
        out.addMessage("/dpo/amp/val", pow(sin_area_phase, 2), 20  ); // area_sum * area_sum + scale(dist_sum, dmin, dmax, 0.00, 1 - area_sum)
